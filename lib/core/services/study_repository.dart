...
class StudyRepository {
  ...
  List<SemesterRecord> semesters() {
    final List<SemesterRecord> saved = LocalStore.instance
        .readList(StoreKeys.gpaSemesters)
        .map(SemesterRecord.fromJson)
        .toList()
      ..sort((SemesterRecord a, SemesterRecord b) =>
  ...
  Future<void> saveSemester(SemesterRecord record) async {
    ...
    final int index =
        current.indexWhere((SemesterRecord s) => s.id == record.id);
    if (index >= 0) {
  ...
  double cumulativeGpa() {
    ...
    final int units =
        all.fold(0, (int sum, SemesterRecord s) => sum + s.totalUnits);
    if (units == 0) return 0;
  ...
  Future<void> _mirrorSemester(SemesterRecord record) async {
    ...
    try {
      await SupabaseService.client.from('gpa_semesters').upsert(<String, dynamic>{
        ...record.toJson(),
  ...
  List<CbtAttempt> attempts() {
    final List<CbtAttempt> saved = LocalStore.instance
        .readList(StoreKeys.cbtAttempts)
        .map(CbtAttempt.fromJson)
        .toList()
      ..sort((CbtAttempt a, CbtAttempt b) => b.takenAt.compareTo(a.takenAt));
    return saved;
  }
  ...
  Future<void> saveAttempt(CbtAttempt attempt) async {
    final List<Map<String, dynamic>> current =
        LocalStore.instance.readList(StoreKeys.cbtAttempts)
    ...
    final List<Map<String, dynamic>> trimmed =
        current.length > 200 ? current.sublist(0, 200) : current;
    await LocalStore.instance.writeList(StoreKeys.cbtAttempts, trimmed);

    if (SupabaseService.isReady) {
      try {
        await SupabaseService.client.from('cbt_attempts').insert(<String, dynamic>{
          ...attempt.toJson(),
          'user_id': SupabaseService.currentUser?.id,
        });
      } catch (error) {
  ...
  int totalQuestionsAnswered() => attempts().fold(
        0,
        (int sum, CbtAttempt a) => sum + a.answers.length,
      );
}
